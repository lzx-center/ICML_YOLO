...
\Big[\mathrm{IoU}(box_i, box_{\mathrm{gt}})\cdot \mathbb{I}_{\mathrm{Class}}(box_i)\Big] < \tau,
\end{equation*}

where $\mathbb{I}_{\mathrm{Class}}(box_i) \triangleq \mathbb{I}(\mathrm{Class}(box_i) = \mathrm{Class}(box_{\mathrm{gt}}))$, and \(\mathbb{I}(\cdot)\) denotes an indicator function (returns $1$ if true, \(0\) otherwise).  
...
\begin{definition}[OD PAC-Verification Problem]  
    ...
    \begin{equation*}
    \mathrm{P}_{\boldsymbol{x}\sim \mathcal{C}}\!\left(
    \exists\, box_i \in \mathrm{N}(\F(\boldsymbol{x})) \ \text{s.t.}\
    \mathbb{I}_{\text{OD}}(box_i)=1
    \right) \ge 1-\alpha ,
    \end{equation*}
    where
    $
    \mathbb{I}_{\text{OD}}(box_i) \triangleq
    \mathbb{I}\!\left(
    \mathrm{IoU}(box_i, box_{\mathrm{gt}})\mathbb{I}_{\mathrm{Class}}(box_i)\ge \tau \right).
    $
...
\begin{figure}[t]
    \centering
    \includegraphics[page=1,width=\columnwidth, clip, trim=0 115 0 98]{image/framework.pdf}
...
\begin{aligned}
    c_1 = &\min_{c\ge0} c  \\
          & \mathrm{s.t.} \quad |\F(\boldsymbol{x}^{(i)})-\F(\boldsymbol{x}^{(0)})|\le c\boldsymbol{v}_{\max}, \forall i \in [N_2]
\end{aligned}
\end{equation}
...
The threshold can be calculated as
\[
\tau(i,\mathcal{Z},box_{\mathrm{gt}})
= \min\big\{\tau_1(i,\mathcal{Z},box_{\mathrm{gt}}),\,\tau_2(i,\mathcal{Z},box_{\mathrm{gt}})\big\},
\]
where $q=\mathrm{Class}(box_{\mathrm{gt}})$ and
\begin{align*}
\tau_1(i,\mathcal{Z},box_{\mathrm{gt}})
&\triangleq
\min_{k\in\Delta}
\mathrm{IoU}(box_i^k,box_{\mathrm{gt}})\cdot
\mathbb{I}_{\mathrm{valid}}(k,i)
\end{align*}
\begin{align*}
\hat{\tau}_2(i,\mathcal{Z},box_{\mathrm{gt}})
&\triangleq
\mathop{\inf}\limits_{\substack{k\in\Delta\\ n\neq i\\ \mathbb{I}_{\mathrm{conf}}(k,n,i)=1}}
\mathrm{IoU}(box_n^k,box_{\mathrm{gt}}),\\
\tau_2(i,\mathcal{Z},box_{\mathrm{gt}})
&\triangleq \min\big\{1,\,\hat{\tau}_2(i,\mathcal{Z},box_{\mathrm{gt}})\big\},
\end{align*}
Here the indicator terms are defined as
\begin{align*}
\mathbb{I}_{\mathrm{valid}}(k,i)
\triangleq&
\mathbb{I}_{\mathrm{Class}}(box_i^k)\cdot
\mathbb{I}(c_i^k\ge\iota),\\
\mathbb{I}_{\mathrm{conf}}(k,n,i)
\triangleq&
\mathbb{I}(c_n^k\ge\iota)\cdot
\mathbb{I}_{\mathrm{Class}}(box_i^k) \\
&\cdot\mathbb{I}(\mathrm{IoU}(box_i^k,box_n^k)\ge\eta)\cdot
\mathbb{I}(c_n^k\ge c_i^k).
\end{align*}
\end{lemma}
